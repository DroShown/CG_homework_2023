...
private:
    ...
    {
        std::map<float,float> height_radius_map;
        // data to fill
        ...
        for(unsigned int i = 0; i < mesh->mNumVertices; i++)
        {
            ...
            if(vector.y > max_y)
                max_y = vector.y;
            height_radius_map[vector.y] = vector.x*vector.x + vector.z*vector.z;
            // normals
            if (mesh->HasNormals())
            {
        ...
        cout << "height radious\n";
        for(auto it = height_radius_map.begin(); it != height_radius_map.end(); ++it)
        {
